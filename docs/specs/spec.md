...
| TBD | 2026-08-08 | #199 landed and `docs/specs/foxio/JA4SSH.md` holds the second transcription written under the #195 method. Eleven rules carry two FoxIO-authored corroborations each, and one is marked uncertain. **FoxIO published a text specification of JA4SSH, and one commit deleted it.** `technical_details/JA4SSH.md` exists at commit `16850cc2c8bcb8328c1a43a851a3a9a6eaa56103` as blob `e9bb04b11a06ee1dfd3dbeaa18b45a0138bcdf48`, 2288 bytes, and commit `b6f3ff4` titled `Update README.md` removed it. **The pinned commit points at the deleted file**: `rust/ja4/src/ssh.rs:283` cites it for the mode rule, so FoxIO's own implementation treats it as the statement of intent. **The same commit deleted seven files**, `JA4.md`, `JA4H.md`, `JA4L.md`, `JA4S.md`, `JA4SSH.md`, `JA4T.md` and `JA4X.md`; the first two exist again at the pinned commit and the other five do not, so **five methods hold a deleted text specification that no transcription has read**, and #221 owns that work. A deleted file corroborates and never outranks the image, and `docs/specs/foxio/README.md` states the rank. **The deleted text states two rules the image only draws.** The image captions part a `Mode of Client Packet Length`, and the text states `Reminder: We’re looking at the TCP payload lengths, not the packet length. In wireshark this is under “tcp.len”.`; a reader of the image alone would read the frame length. The text also excludes a retransmission from the packet count, with the words `TCP replays`. **The three declined reference behaviours all stand, and the specification contradicts two of them.** #96 is confirmed at `python/ja4ssh.py:8-15` with `python/ja4ssh.py:88` and `python/ja4ssh.py:128`: `dict(ja4sh_stats)` is a shallow copy, so one pair of payload lists serves every window of every connection, and the text's own heading reads `How to measure the mode for TCP payload lengths across 200 packets in the session`. #97 is confirmed at `python/ja4ssh.py:125`, which runs `(entry['count'] % ssh_sample_count) == 0` on every packet while `python/ja4ssh.py:98-99` advances the count only for an SSH packet, so a bare ACK arriving between packet 200 and packet 201 writes a second occurrence; `rust/ja4/src/ssh.rs:271-274` holds the same empty-window guard that `ja4plus/fingerprinters/ja4ssh.py:342-343` holds, so the Rust reference contradicts the Python one. #105 is confirmed at `python/ja4.py:370-377`, where `if stream:` reads the stream index `0` as false. **The specification states the window boundary and states nothing about what closes the last window**, which is the reading #214 needs. The image states `(runs every 200 SSH packets by default)` and the deleted text states `Runs every n packets per SSH TCP stream. n = 200 by default but is configurable.`; neither names a FIN packet, a connection that closes, or the end of a capture. **The references split two against two.** `python/ja4.py:554-556` and `wireshark/source/packet-ja4.c:1399-1404` close the last window on a FIN+ACK packet, and `rust/ja4/src/ssh.rs:45-55` and `zeek/ja4ssh/main.zeek:160-164` close it at the end of the capture; `ja4plus/fingerprinters/ja4ssh.py:221-222` follows the first pair. **`python/ja4.py:610` runs no end-of-capture step at all**, because the line `#finalize_ja4ssh() if 'ja4ssh' in output_types else None` is commented out. **`ssh2.pcapng` carries no FIN+ACK packet on port 22**, so its connection holds its last window open when the capture ends, and this is the whole mechanism of #214. **`ja4plus` already counts that window and matches two references byte for byte**: it holds 42 client SSH packets, 76 server SSH packets, 51 client ACKs and 2 server ACKs, which would give `c36s52_c42s76_c51s2`. **`tests/foxio_vectors/rust_expected/ja4__insta@ssh2.pcapng.snap:215-217` already sits in this repository and holds that exact value**, which equals the Zeek baseline value; #198 did not read it. The Python second value `c36s36_c0s0_c2s0` is the #97 defect and not a trailing window. **`tests/test_foxio_rust_parity.py:72` reads `SNAPSHOT_METHODS = (("JA4", "ja4"), ("JA4S", "ja4s"))`, so the harness parses that snapshot and never reads its `ja4ssh` field.** That is the fourteenth instance of a comparison that is never made. Four disagreements with `ja4plus` are recorded, and none moves a fingerprint: the trailing window of D1, which #214 holds; `ja4ssh.py:135` reading SSH on any port where `python/ja4.py:549` requires port 22; the three endpoint rules of `ja4ssh.py:284-326`, which no specification states; and `ja4ssh.py:214` testing `>=` where two references test equality and the Zeek script also tests `>=`. No file under `ja4plus/` changed and no fingerprint moved. `tests/foxio_deviations.json` holds 120 keys before and after, against 120 `xfailed` cases. |
