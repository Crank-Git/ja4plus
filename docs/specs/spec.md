...
| TBD | 2026-08-07 | #182 landed and `interpret_fingerprint` rejects a JA4SSH part that carries no client prefix. The method read a client value with `part[1:]` and never checked that the removed character is `c`. The part `36s36` therefore reported a client packet size of 6, and the caller read a number the fingerprint does not hold. **The issue body named the client packet size alone, and the measurement found the same defect in all three parts.** `c36s36_50s50_c70s30` reported a client SSH ratio of 0, and `c36s36_c50s50_70s30` reported a client ACK ratio of 0. A wrong prefix passed as well: `x36s36_c50s50_c70s30` reported a client packet size of 36. The method now returns `{"error": "Invalid JA4SSH format"}`. That is the error it already returns for a wrong part count, so the change adds no second error shape. **The issue body's line number is stale and the plan corrected it:** the two lines are 479 and 480, not 352. **Line 480 holds no defect, and the plan expected one.** A JA4SSH part names its server value after the `s` separator. It carries no server prefix, so `split("s")[1]` removes no character. A part that holds no `s` already raises `IndexError` and returns the error dictionary. `interpret_fingerprint` reads a fingerprint and produces none, so no FoxIO vector covers it. No fingerprint moved, and `tests/foxio_deviations.json` holds 120 keys before and after, against 120 `xfailed` cases. **#186 opens for one finding this issue placed out of scope.** A part that carries a second `s` separator, such as `c36s36s99`, passes the method, because nothing checks that the split produced two elements. A comment on #186 records three readings of the same class: `int()` accepts a space, a sign, and a Unicode digit, so `c 36s36`, `c-5s36` and `c٦s36` all pass. |
| TBD | 2026-08-08 | #175 landed and the HTTP cookie parser holds a maximum entry count and two maximum lengths: 512 cookie pairs, 256 bytes of cookie name and 4096 bytes of cookie value. **Past any bound the cookie parse produces no result, and the request therefore produces no JA4H value.** The user decided that on 2026-08-08, and the reason is specific to this fingerprint. The JA4H cookie hash reads the content and the order of the list. A truncated list therefore produces a value that compares **equal** to a different sender's. A tool whose purpose is to compare one output against another must not emit a value that describes traffic the sender did not send. Absent output is safer than silently wrong output. `ja4plus/utils/ssh_utils.py:242` already held that shape past `MAX_SSH_BANNER_BYTES`, and the truncating style at `ja4plus/fingerprinters/ja4x.py:334` stays where it is, because a stopped certificate read fails to parse rather than producing a comparable value. The accepted cost is stated and is not a defect: a sender suppresses its own JA4H value with a large Cookie header, which the SSH banner bound already permits. **The issue body named two files and the defect sat in three call sites.** `_extract_http_info_from_bytes` in `ja4plus/fingerprinters/ja4h.py` holds the third, and it is the site the JA4H reassembled-stream path reads, so a fix that covered the two named files would have left the stream path uncapped. That is the same shape as #35, which the body of this issue records, and the fifth issue premise this session that did not survive contact with the code. **The vectors could not settle the policy, and the worker returned `needs-feedback` rather than choosing.** The largest readings across `tests/foxio_vectors/` are 14 cookie pairs, a 41-byte name and a 264-byte value, over 6 requests of the 82 that carry a cookie, so no vector reaches any bound and the two candidate policies produce identical output on the whole vector set. Acceptance criterion 1 also contradicted the issue body: it named a bounded list, which is truncation, while the body named absent output. The criterion is rewritten. **The growth the body reports is confirmed on the base commit `b6468dc`:** a header of 100000 pairs built 100000 entries in 11.0 to 12.1 ms on all three sites, and one cookie produced a name of 1000000 bytes and a value of 1000000 bytes, so neither the name nor the value carried a bound either. `TCPStreamReassembler` already refused a segment past `max_stream_bytes=1048576`, which held the list near 260000 pairs; that is an outer bound and not the entry count the state rule states. **The bound covers the list the parser builds, not the value it returns.** `parse_cookie_header` reads the header through a generator and stops one segment past the cap, so a header of 100000 pairs allocates 513 segments rather than 100000; `str.split` would build the whole list before refusing it. The length bounds read the UTF-8 byte count, because the parsers decode with `errors="ignore"` and a character count accepts a name of four times the stated size. **Each bound is proven by removing it:** the pair cap fails 4 cases, the name cap 2, the value cap 2, the byte count 1, the generator 1, and the third call site 5. Removing all three guards at once fails 8 of the 15 cases. **The enforcement was proven and the three decided numbers were not, which is the thirteenth instance of this project's signature defect and the second on this issue.** Every boundary case builds its input from the constant, as `MAX_COOKIE_PAIRS + 1`, so each proves that the refusal happens at the constant and none reads the constant. The project manager measured the gap rather than reading the verdict: raising all three constants to 1000000 left 12 of 14 cases green. A later `MAX_COOKIE_PAIRS = 5` would then suppress the JA4H value of ordinary traffic against a green suite. Building the input from the constant is the right way to write a boundary case, so the cases stay and `test_the_three_bounds_hold_the_values_the_user_decided_on_2026_08_08` now reads the three numbers and cites the decision comment. Changing any one of the three fails that case alone and leaves the other 14 green. **No JA4H value moved, measured rather than asserted:** every capture under `tests/foxio_vectors/` was replayed through `JA4HFingerprinter` on the base and on the branch, 82 values across 8 captures, and 0 captures differ on the hashed form or on the raw original-order form. `tests/foxio_deviations.json` holds 120 keys before and after, against 120 `xfailed` cases. **#188 opens for one finding this issue placed out of scope.** The same three parsers hold no bound on the header list, and a request of 100000 header lines builds 100000 entries in 38.2 ms. It takes the same policy when it is scheduled. |
