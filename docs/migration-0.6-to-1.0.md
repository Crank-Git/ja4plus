...
| The result of a lookup | `JA4DBClient.lookup` returns a dictionary with the keys `application`, `type` and `notes`. | It returns a frozen `LookupResult`, which adds the `source` field. Item access still works and it raises a `DeprecationWarning`. | The typed result matches `lookup.go:23` of the port, under parity rule 2. | Round 122, #59 and #364 |
