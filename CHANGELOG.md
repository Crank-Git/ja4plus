...
- **The marker rule states the decided-entry count the register holds, and a case
  ...
  drift at 129, and #272 removed one entry and decided five inside this batch. **The
  ...
  now reads the two counts out of the prose and measures them against the register, so
  the next move of the register fails a case rather than sitting in a docstring. The
  case failed on the unchanged prose with `assert 128 == 134`, and three reverted
  mutations prove that it fails when the register moves and the prose does not. No
  register entry changed, no file under `ja4plus/` changed, and no fingerprint moved.
