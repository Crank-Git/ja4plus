...
- **The built wheel carries no file under `docs/`** (#455). Round TBD.
  ...
  **The repair adds two entries to the one list that already excludes `tests/` and
  ...
  `["tests", "tests.*", "examples", "docs", "docs.*"]`. A pattern matches one package and
  no subpackage, so the entry `docs.*` carries `docs.specs` and `docs.specs.features`. The
  ...
  **The tests came first.**
  ...
  `test_the_source_distribution_carries_the_documentation_tree` hold the two states a wrong
