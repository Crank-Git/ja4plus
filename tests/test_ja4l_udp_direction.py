...
def test_the_server_value_appears_when_the_client_address_sorts_first():
    ...
    two Initial packets give it none. The client Handshake packet that follows gives it.
    """
    fingerprinter = JA4LFingerprinter()
    fingerprinter.process_packet(
        _quic_packet("10.0.0.1", "10.0.0.2", 50000, 443, QUIC_INITIAL, t=0.0)
    )
    initial_result = fingerprinter.process_packet(
        _quic_packet("10.0.0.2", "10.0.0.1", 443, 50000, QUIC_INITIAL, t=0.001)
    )
    fingerprinter.process_packet(
        _quic_packet("10.0.0.2", "10.0.0.1", 443, 50000, QUIC_HANDSHAKE, t=0.003)
    )
    result = fingerprinter.process_packet(
        _quic_packet("10.0.0.1", "10.0.0.2", 50000, 443, QUIC_HANDSHAKE, t=0.005)
    )
...
def test_the_server_value_appears_when_the_client_address_sorts_last():
    ...
    fingerprinter.process_packet(
        _quic_packet("10.0.0.1", "10.0.0.99", 443, 50000, QUIC_HANDSHAKE, t=0.005)
    )
    result = fingerprinter.process_packet(
        _quic_packet("10.0.0.99", "10.0.0.1", 50000, 443, QUIC_HANDSHAKE, t=0.008)
    )
    assert initial_result is None
...
def test_the_client_value_measures_the_two_handshake_packets():
    ...
    The packet that fills point `D` gives two values. `process_packet` returns the
    server value, and it holds the client value in `last_extra_fingerprints`.
    """
    fingerprinter = JA4LFingerprinter()
    ...
    assert initial_server is None
    assert handshake_server is None
    assert handshake_client == "JA4L-S=1000_64_quic"
    assert fingerprinter.last_extra_fingerprints == ["JA4L-C=1000_64_quic"]
